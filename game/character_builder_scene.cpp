...
void CharacterBuilderScene::on_enter() {
    ...
    for (const auto& id : race_ids_) {
        if (auto race = race_registry.get(id)) {
            race_names_.push_back(race->name);
        }
    }

    class_ids_ = class_registry.get_all_ids();
    for (const auto& id : class_ids_) {
        if (auto char_class = class_registry.get(id)) {
            class_names_.push_back(char_class->name);
        }
    }
}
...
void CharacterBuilderScene::setup_ui() {
    ...
    // Finish button
    finish_button_ = Button("Start Game", [this] { finish_character_creation(); });

    // Create scrollable info panel
    info_panel_ = Renderer([] { return text(""); });
...
Component CharacterBuilderScene::get_component() {
    return Renderer(main_container_, [this] {
        ...
        auto name_section = vbox({
            text("Name") | bold,
            separator(),
            name_input_->Render()
        }) | border | (name_panel_->Focused() ? color(Color::Cyan) : color(Color::White));

        auto gender_section = vbox({
            text("Gender") | bold,
            separator(),
            gender_selector_->Render()
        }) | border | (gender_panel_->Focused() ? color(Color::Cyan) : color(Color::White));
        ...
        if (race_names_.empty()) {
            ...
            race_section = vbox({
                text("Race") | bold,
                separator(),
                race_selector_->Render()
            }) | border | (race_panel_->Focused() ? color(Color::Cyan) : color(Color::White));
        }
        ...
        if (class_names_.empty()) {
            ...
            class_section = vbox({
                text("Class") | bold,
                separator(),
                class_selector_->Render()
            }) | border | (class_panel_->Focused() ? color(Color::Cyan) : color(Color::White));
        }
        ...
        auto input_panels = vbox({
            top_row,
            separator(),
            bottom_row,
            separator(),
            finish_button_->Render() | center
        });
        ...
        auto info_section = vbox({
            text("Information") | bold | underlined | center,
            separator(),
        ...
        auto content = hbox({
            input_panels | flex,
            separator(),
            info_section | size(WIDTH, EQUAL, 50)
        });
...
std::string CharacterBuilderScene::get_info_text_for_panel(FocusedPanel panel) {
    switch (panel) {
        ...
        case FocusedPanel::GENDER:
            return "Select your character's gender. This is primarily for roleplay purposes and does not affect gameplay mechanics.";
...
void CharacterBuilderScene::finish_character_creation() {
    ...
    std::transform(character_data_.gender.begin(), character_data_.gender.end(),
    ...
    get_engine()->get_logger()->info("Character created: {} ({}) - Race: {}, Class: {}",
        character_data_.name, character_data_.gender,
        race_names_[race_index_], class_names_[class_index_]);
